...
].map((json) => Elemento.fromJson(json)).toList(growable: false);

class GameElement extends StatelessWidget {
  final int index;

  const GameElement({Key? key, required this.index}) : super(key: key);
        ...
        child: Stack(
            children: <Widget>[
              Positioned(
                  ...
                  top: 164,
                  left: 151,
                  child: Text(tablaPeriodicaJson[index].simbol, textAlign: TextAlign.left, style: TextStyle(
                      color: Color.fromRGBO(0, 0, 0, 1),
                      fontFamily: 'Inter',
                      fontSize: 128,
                      letterSpacing: 0 /*percentages not used in flutter. defaulting to zero*/,
